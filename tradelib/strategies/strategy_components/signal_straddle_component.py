...
from tradelib_global_constants import underlying, tolerance, strict_condor, strict_tolerance, steps, unit_size, unwind_time, expiry_info, gamma_hedge, rsi_window, rsi_upper, rsi_lower, rsi_variable, rsi_daily_entry_limit
from tradelib_blackscholes_utils import getInstrumentDetailsWithBlackscholes

class SignalStraddleComponent(StrategyComponent):
    # TODO: have lifecycle hooks for components
    def __init__(self, trading_platform: TradingPlatform, portfolio: Portfolio, blotter: Blotter, skip_count:int, outstrike: float, trade_side, entry_cool_off_time, exit_cool_off_time, signal_strat_time, signal_end_time, signal_tolerance, fast_lookback_period, slow_lookback_period, execute_on_day_start:bool=True, contract_quota: int = None) -> None:
        super().__init__("signal_straddle_component", trading_platform, portfolio, blotter, skip_count, execute_on_day_start, trade_side, contract_quota)
        ...
        self.vs_queue = deque(maxlen=self.rsi_window+1)
        self.iv_queue = deque(maxlen=self.rsi_window+1)
        # self.rv_queue = deque(maxlen=self.slow_lookback_period)
        # self.spot_queue = deque(maxlen=self.slow_lookback_period)
        self.vr_queue = deque(maxlen=self.rsi_window+1)
    ...
    def get_signal(self, timestamp: datetime):
        ...
        try:
            ...
            if ((rsi_variable == 'vs') or (rsi_variable == 'vr')):
            ...
            elif rsi_variable == 'vr':
                rsi_input = self.vr_queue
            elif rsi_variable == 'iv':
                rsi_input = self.iv_queue
            

            if len(rsi_input) <= (self.rsi_window):
                #If the queue is not fullfiled return the do nothing signal
                logger.warning(f"At time {timestamp} the len of {rsi_variable} queue {len(list(rsi_input))} is less then equal to {self.rsi_window = }")
                return 0
            
            # rsi_list = talib.RSI(np.array(list(rsi_input).reverse()), timeperiod=self.rsi_window)
            rsi_list = talib.RSI(np.array(list(rsi_input)[::-1], dtype=np.float64), timeperiod=self.rsi_window)
            rsi_val = rsi_list[-1]
            logger.info(f"At time {timestamp} for RSI input {rsi_input} {rsi_list=} {rsi_val=} {self.rsi_upper=} {self.rsi_lower=}")

            if rsi_val is None:
                logger.warning(f"At time {timestamp} RSI val is None for RSI input {rsi_input} {rsi_val=} {self.rsi_upper=} {self.rsi_lower=}")
                return 0 

            exit_bool = rsi_val < self.rsi_lower
            entry_bool = rsi_val > self.rsi_upper
            ...
            if exit_bool==True and entry_bool==False:
                ...
                logger.info(f"At time {timestamp} for RSI input {rsi_input} {rsi_val=} {self.rsi_upper=} {self.rsi_lower=} {exit_bool=} {entry_bool=}")
                return 1
