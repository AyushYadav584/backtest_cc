...
from tradelib_trade_utils import get_atm_option, get_delta_based_otm_option, get_actual_expiry_dates, get_early_expiry_date, get_late_expiry_date, is_trading_date
...
from tradelib_global_constants import underlying, tolerance, strict_condor, strict_tolerance, steps, unit_size, unwind_time, expiry_info, day_of_week_signal_strength, delta_atm_outstrike, delta_condor_trade_stop_time, delta_condor_trade_start_time

class DayofWeekDeltaCondorComponent(StrategyComponent):
    ...
    def __init__(self, trading_platform: TradingPlatform, portfolio: Portfolio, blotter: Blotter, skip_count:int, delta_outstrike: float, tolerance: int, strict_condor:bool, strict_tolerance: bool, underlying:str, unit_size:int, unwind_time, trade_side, execute_on_day_start:bool=True ) -> None:
        super().__init__("day_of_week_delta_condor_component", trading_platform, portfolio, blotter, skip_count, execute_on_day_start, trade_side)
    ...
    def generate_trades(self, timestamp: datetime) -> List[Trade]:
        ...
        try:
            ...
            for nearest_expiry_date in component_expiry_list:
                ...
                atm_pe = get_delta_based_otm_option(self.trading_platform, self.underlying, delta_atm_outstrike, self.tolerance, self.strict_tolerance, "PE", nearest_expiry_date, timestamp, self.logger)
                if (atm_pe == None):
                ...
                atm_ce = get_delta_based_otm_option(self.trading_platform, self.underlying, delta_atm_outstrike, self.tolerance, self.strict_tolerance, "CE", nearest_expiry_date, timestamp, self.logger)
                if atm_ce == None:
