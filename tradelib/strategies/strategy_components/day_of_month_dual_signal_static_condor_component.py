...
class DayofMonthDualSignalStaticCondorComponent(StrategyComponent):
    ...
    def __init__(self, trading_platform: TradingPlatform, portfolio: Portfolio, blotter: Blotter, skip_count:int, outstrike: float, trade_side, execute_on_day_start:bool=True) -> None:
        super().__init__("day_of_month_dual_signal_static_condor_component", trading_platform, portfolio, blotter, skip_count, execute_on_day_start, trade_side)
    ...
    def generate_signal(self, timestamp:datetime):
        try:
            ...
            if self._nearest_expiry != nearest_exp_date:    
                ...
                month_signals_dict = {}
                # {datetime(2024,1,1).date : (2,45)}

                while iter_date >= first_trading_day:
                    # use is trading day function
                    if is_trading_date(date=iter_date, trading_platform=self.trading_platform):
                    # if True:
                        strenght_val = day_of_month_signal_strength.get(day_count)
                        freq_val = day_of_month_signal_frequency.get(day_count)
                        month_signals_dict[iter_date] = (strenght_val, freq_val)
                        day_count += 1
    ...
    def generate_trades(self, timestamp: datetime) -> List[Trade]:
        ...
        try:
            ...
            trade_mul = self.get_day_of_month_signal(timestamp=timestamp)[0]
            unit_size = self.unit_size * trade_mul

            # set trade size based on signal
            trade_interval = self.get_day_of_month_signal(timestamp=timestamp)[1]
