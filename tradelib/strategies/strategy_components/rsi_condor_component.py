...
from tradelib_global_constants import underlying, tolerance, strict_condor, strict_tolerance, steps, unit_size, unwind_time, expiry_info, gamma_hedge, \
    rsi_window, rsi_upper, rsi_lower, rsi_variable, rsi_daily_entry_limit
from tradelib_blackscholes_utils import getInstrumentDetailsWithBlackscholes

class RsiCondorComponent(StrategyComponent):
    ...
    def __init__(self, trading_platform: TradingPlatform, portfolio: Portfolio, blotter: Blotter, skip_count:int, delta_outstrike: float, trade_side, entry_cool_off_time, exit_cool_off_time, signal_strat_time, signal_end_time, signal_tolerance, fast_lookback_period, slow_lookback_period, execute_on_day_start:bool=True, contract_quota: int = None) -> None:
        super().__init__("rsi_condor_component", trading_platform, portfolio, blotter, skip_count, execute_on_day_start, trade_side, contract_quota)
    ...
    #         if len(self.iv_queue) <= (self.rsi_window):
    #             #If the queue is not fullfiled return the do nothing signal
    #             logger.warning(f"At time {timestamp} the len of vr_queue {len(list(self.iv_queue))} is less then equal to {self.slow_lookback_period =} or {self.rsi_window = }")
    #             return 0
