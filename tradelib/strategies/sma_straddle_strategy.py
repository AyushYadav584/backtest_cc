...
from tradelib.strategies.strategy_components.sma_straddle_component import SMAStraddleComponent
...
from tradelib_global_constants import trade_interval_time, hedge_interval_time, OTM_outstrike, unwind_trade_side, strategy_trade_side, \
                                      hedge_trade_side, m2m_side, hedge_by_option, signal_strat_time, signal_end_time, entry_cool_off_time, \
                                          exit_cool_off_time, signal_tolerance, slow_lookback_period, fast_lookback_period, sma_contract_quota, sma_lower, sma_upper, sma_variable, sma_window

class SMAStraddleStrategy(TradeStrategy):
    def __init__(self, trading_platform: TradingPlatform, portfolio: Portfolio, blotter: Blotter) -> None:
        super().__init__("sma_straddle_strategy", trading_platform, portfolio, blotter)

        self.m2m_side = m2m_side

        unwind_component = UnwindComponent(trading_platform, self.portfolio, self.blotter, unwind_trade_side, contract_quota=sma_contract_quota)
        ...
        strategy_component = SMAStraddleComponent(trading_platform=trading_platform,
                                                    portfolio=self.portfolio,
                                                    blotter=self.blotter,
                                                    skip_count=1,
                                                    outstrike=OTM_outstrike,
                                                    ...
                                                    contract_quota=sma_contract_quota,
                                                    signal_window = sma_window,
                                                    signal_upper = sma_upper,
                                                    signal_lower = sma_lower,
                                                    signal_variable = sma_variable,
                                                    )
